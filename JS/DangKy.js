$(document).ready(function () {
  ...
  ten.on("blur", function () {
    ...
    if (ten.val() === "") {
      tbTen.text("(*) Tên và tên đệm không được rỗng");
      check5 = false;
    } else if (regex.test(ho.val())) {
      tbTen.text("(*)");
      check5 = true;
  ...
  //   if (hoChieu.val() == null) {
  //     tbHoChieu.text("(*) Hộ chiếu/CCCD không được rỗng");
  //     check10 = false;
  //   } else {
  //     tbHoChieu.text("(*)");
  //     check10 = true;
  ...
  btnDangKy.on("click", function () {
    if (luaChon.is(":checked")) {
      ...
      if (
        ...
        var maTokenRandom = randomMaToken();
        var soHoiVienRandom = randomSoHoiVien();
        var table =
          "<tr><td>" +
          soHoiVienRandom +
          "</td><td>" +
